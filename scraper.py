...
# Phase 3's JSON source: /web-api + the college's page path returns that page's
# pageProps.data — basic_info and schemaJsonLd included. Found 2026-09-22 when
# the HTML college pages were being refused 86-100% while web-api ran at ~6%.
COLLEGE_API_BASE = "https://collegedunia.com/web-api"
...
def college_api_url(link: str) -> str:
    """/web-api + the page path. Accepts a stored absolute URL, a bare path, or a
    301 `target` (always absolute)."""
    link = (link or "").strip()
    path = urlparse(link).path if "://" in link else link.split("?")[0]
    if not path.startswith("/"):
        path = "/" + path
    return COLLEGE_API_BASE + path
...
class PageGoneError(Exception):
    """The site says this page does not exist ({"status": 404} from web-api).

    Distinct from BlockedError on purpose: a 403 is the site refusing US and is
    worth retrying through another IP; a 404 is the site telling us the page is
    not there, and retrying it five times only burns requests. Not, on its own,
    grounds to flag a college as gone — see the 2026-09-21 canary notes on
    reading too much into one run."""


class PayloadMismatchError(Exception):
    """A web-api response that parsed but is not the college we asked for —
    missing basic_info, or basic_info.id != college_id. Nothing is written."""
...
class Client:
    ...
    def get_json(self, url: str, label: str) -> Dict[str, Any]:
        """GET a web-api URL with the usual retry / rotation / adaptive handling.

        Returns the parsed JSON as-is. Unlike fetch_listing/fetch_courses_list it
        does NOT treat {"status": 301|404} as a block: those are answers, not
        refusals, and the caller decides what they mean. A 404 sentinel raises
        PageGoneError immediately rather than being retried."""
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                ctype = resp.headers.get("Content-Type", "")
                if "json" not in ctype and not resp.text.lstrip().startswith("{"):
                    raise BlockedError("non-JSON web-api response (challenge page?)")
                data = resp.json()
                if not isinstance(data, dict):
                    raise ValueError("web-api returned non-object JSON")
                if data.get("status") == 404:
                    raise PageGoneError(f"{label}: web-api status 404")
                self.pm.report_success(proxy)
                if self.adaptive:
                    self.adaptive.on_success()
                if self.verbose:
                    self.log(f"   · API …{url[-46:]} → {len(resp.content or b'')//1024} KB")
                return data
            except PageGoneError:
                self.pm.report_success(proxy)   # the proxy did its job
                raise
            except (BlockedError, requests.RequestException, ValueError) as err:
                last_err = err
                self._on_failure(err, proxy, attempt, label)
        raise RuntimeError(f"{label} failed after {self.max_retries} attempts: {last_err}")

    def fetch_college_api(self, link: str, college_id: Any,
                          max_hops: int = 2) -> Dict[str, Any]:
        """A college page's data via web-api instead of HTML.

        Moved colleges (e.g. IIT Bombay, /college/… -> /university/…) come back
        as HTTP 200 + {"status": 301, "target": "<new url>"}; the HTML path
        followed that redirect silently, so this follows it too, up to max_hops.
        The stored link is NOT rewritten — parity with the HTML path.

        Before anything is returned, basic_info.id must equal college_id. That
        one check guards against every wrong-payload case at once: a redirect
        landing somewhere unexpected, an error body shaped like success, a
        different college entirely."""
        url = college_api_url(link)
        label = f"API college {college_id}"
        for _hop in range(max_hops + 1):
            data = self.get_json(url, label)
            if data.get("status") in (301, 302, 307, 308) and data.get("target"):
                url = college_api_url(str(data["target"]))
                continue
            bi = data.get("basic_info")
            if not isinstance(bi, dict) or not bi:
                raise PayloadMismatchError(f"{label}: response has no basic_info")
            if college_id is not None and _to_int(bi.get("id")) != _to_int(college_id):
                raise PayloadMismatchError(
                    f"{label}: basic_info.id={bi.get('id')} is not the college requested")
            return data
        raise PayloadMismatchError(f"{label}: more than {max_hops} redirects")
...
def parse_college_basic_info(html: str) -> Dict[str, Any]:
    ...
    return basic_info_from_data((pp or {}).get("data"))


def basic_info_from_data(data: Any) -> Dict[str, Any]:
    """The basic_info extraction itself, shared by both Phase 3 sources.

    `data` is the college page's pageProps.data. The HTML path digs it out of
    __NEXT_DATA__; the JSON path gets it directly, because
    /web-api/college/<id>-<slug> returns exactly that object (verified field for
    field against the HTML on 2026-09-22). One function, so the two sources
    cannot drift apart.
    """
    bi = (data or {}).get("basic_info") if isinstance(data, dict) else None
    if not isinstance(bi, dict) or not bi:
        return {}
...
def _ld_items(obj: Any) -> List[Any]:
    """Flatten one JSON-LD block into its items: a list, an @graph, or a single
    object. Strings are parsed — the web-api returns some blocks as JSON text."""
    ...
    if isinstance(obj, list):
        out: List[Any] = []
        for o in obj:
            out.extend(_ld_items(o))
        return out
    if isinstance(obj, dict) and isinstance(obj.get("@graph"), list):
        return _ld_items(obj["@graph"])
    return [obj]


def _ld_college_fields(items: Iterable[Any]) -> Dict[str, Any]:
    """The Phase-3 fields from the first CollegeOrUniversity item. Shared by the
    HTML and JSON sources. Deliberately ignores UserComments/reviews."""
    for o in items:
...
def parse_college_ld(html: str) -> Dict[str, Any]:
    ...
    for block in _LD_RE.findall(html or ""):
        items.extend(_ld_items(block))
    return _ld_college_fields(items)


def parse_college_ld_api(data: Any) -> Dict[str, Any]:
    """The same fields from a /web-api/college response. Its JSON-LD lives in
    `schemaJsonLd`, an object keyed "0".."4" (an array in disguise) whose values
    may be objects or JSON strings."""
    s = (data or {}).get("schemaJsonLd") if isinstance(data, dict) else None
    if isinstance(s, dict):
        s = [s[k] for k in sorted(s, key=lambda k: (not str(k).isdigit(),
                                                   int(k) if str(k).isdigit() else 0, str(k)))]
    return _ld_college_fields(_ld_items(s or []))
...
def run_enrichment(job_id: int, cfg: Dict[str, Any], db_path: str = db.DB_PATH,
    ...
    # "api" (default) or "html". The HTML path is kept, not deleted, in case the
    # web-api route changes shape; flip it back with {"phase3_source": "html"}.
    source = str(cfg.get("phase3_source", "api")).lower()
    if source not in ("api", "html"):
        source = "api"
    log(f"Phase 3: enriching {total} colleges, concurrency={concurrency}, "
        f"source={'web-api JSON' if source == 'api' else 'HTML pages'}")
    ...
    def worker():
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                if source == "api":
                    # One JSON request carries both halves: basic_info and the
                    # CollegeOrUniversity JSON-LD. Verified identical to the
                    # HTML's on 2026-09-22, at ~35 KB on the wire vs ~135 KB,
                    # on an endpoint family the site refuses ~6% of the time
                    # rather than the HTML pages' 86-100%.
                    data = client.fetch_college_api(cobj["link"], cobj["college_id"])
                    fields = parse_college_ld_api(data)
                    basic = basic_info_from_data(data) if cfg.get("basic_info", True) else {}
                else:
                    html = client.get_text(cobj["link"])
                    fields = parse_college_ld(html)
                    # The same page also carries __NEXT_DATA__.data.basic_info,
                    # with roughly double the fields. Free: no extra request.
                ...
                # The site says the page does not exist. Logged, not written, and
                # deliberately NOT flagged as gone on one sighting.
                log(f"  college {cobj['college_id']}: page not found (web-api 404)")
                with prog_lock:
                    state["gone"] = state.get("gone", 0) + 1
            except PayloadMismatchError as err:
                log(f"  college {cobj['college_id']}: {str(err)[:90]} — nothing written")
                with prog_lock:
    ...
    try:
        ...
        if state["incomplete"]:
            ...
            if state.get("gone"):
                msg += f", {state['gone']} page-not-found"
            if state.get("mismatch"):
                msg += f", {state['mismatch']} wrong-payload (not written)"
            status = "completed"
        db.update_job(job_id, status=status, message=msg, finished_at=time.time(),
                      req_count=reqs, bytes_count=byts, db_path=db_path)
        log(msg)
