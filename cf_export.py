"""Course Finder — exports (cf_ tables only; never touches any other module).

MEMORY: same streaming design as export.py / sa_export.py. Rows come off the
sqlite cursor one at a time and the workbook uses openpyxl's write_only mode,
so peak RSS is flat no matter how large cf_offerings grows — which matters,
since a complete crawl is ~600k rows on a 2 GB host.
"""
...
TABLES = ("cf_courses", "cf_offerings", "cf_course_progress",
          "cf_partition_progress")
...
def _fetch(table, db_path=cf_db.CF_DB_PATH):
    """Materialises a whole table. Kept only for ad-hoc use — the
    exporters below stream instead."""
    with cf_db.connect(db_path) as conn:
        cur = conn.execute(f"SELECT * FROM {table}")
        cols = [d[0] for d in cur.description]
        return cols, cur.fetchall()


def to_xlsx(tables: Sequence[str] = TABLES, db_path: str = cf_db.CF_DB_PATH,
            include_raw: bool = True, out_path: Optional[str] = None,
            max_rows_per_sheet: int = XLSX_MAX_ROWS):
    """One sheet per cf_ table. Streams; peak memory is independent of size.
    ...
    if truncated:
        ...
        for t, n in truncated:
            print(f"[cf_export] WARNING: sheet '{t}' truncated at {n:,} rows "
...
def to_csv(table, db_path=cf_db.CF_DB_PATH, include_raw: bool = True,
...
def to_json(table, db_path=cf_db.CF_DB_PATH, include_raw: bool = True,
            out_path: Optional[str] = None):
    if out_path:
